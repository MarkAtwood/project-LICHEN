...
Delta = (current_sfn - last_sfn) as uint32_t subtraction (wrapping_sub in Rust) ensures correct wrap behavior per RFC 1982.

Edge case example (0xFFFFFFFF boundary):
```
last_sfn = 0xFFFFFFFFu;
current_sfn = 0x00000002u;
delta = current_sfn - last_sfn;  // = 3 via unsigned wrap
```
...
delta = (current_sfn - last_sfn) & 0xffffffff  # uint32 mask = 3
```
This MUST be treated as advancement of 3 slots. Signed arithmetic would yield negative, breaking desync and scheduling. Test vectors in ccp16.json, test_vectors.py, and ccp16-desync.json MUST cover boundaries (including TrickleTimer imin>0 validation in RPL context).
