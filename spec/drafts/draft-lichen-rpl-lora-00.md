...
**ASSIGNED_SF and RF Metrics Option (CCP-16):** MUST be carried in RPL DIO options per CCP-16. The DAG Metric Container MUST include current SF per-neighbor EMA-derived recommendation (RSSI/SNR EMA alpha=1/4 per CCP-15 from `rust/lichen-core/src/rf_health.rs:19`), density, utilization. Nodes MUST compute TX_SF via pseudocode in physical-link:3.4 and spec/02a-coordinated-capacity.md section 4.2. Thresholds and EMA update normative there. DIOs on control channel announce it. RX scanning on CH0 REQUIRED. Test vectors `test/vectors/ccp16.json` MUST validate interop.
