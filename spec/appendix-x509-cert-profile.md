...
4. **Address binding (the attestation payload):** recomputes the full
   ...
   verifies that the SAN contains exactly one native `0200::/8`
   iPAddress and that it equals this recomputed 128-bit address (not
   merely that its lower 64 bits match the IID). A certificate carrying
   zero or more than one native entry, or whose single native entry does
   not match, MUST be rejected — it attests a key-to-address pairing
   that does not hold. (Issuer behavior is specified in Section 4; this
   step makes the exactly-one rule a verifier requirement so that
   conformance does not depend on CA diligence.)
