...
struct ExpectedCounts {
    ...
    rule255_rx_byte_preserving_address: usize,
    #[cfg_attr(
...
fn test_schc_adaptation_vectors() {
    ...
    let mut rule255_rx_byte_preserving_address_count = 0;
    #[cfg(all(feature = "raw-fragment-codec", feature = "test-utils"))]
    ...
    for vector in &vectors.vectors {
        ...
        match category.as_str() {
            ...
            "rule255_rx_byte_preserving_address" => {
                use std::net::Ipv6Addr;
                use std::str::FromStr;

                rule255_rx_byte_preserving_address_count += 1;
                let source = Ipv6Addr::from_str(
                    vector
                        .source_ipv6
                        .as_deref()
                ...
                .octets();
                assert!(
                    vector.expect_valid.unwrap_or(false),
                    "{}: byte-preserving rows decode successfully",
                    name
                );
                ...
                // Emission policy reports the first bogus endpoint: source
                // defects (unspecified/multicast) before destination ones.
                let fragment = if source.iter().all(|&b| b == 0) || source[0] == 0xff {
                    "source"
                } else {
                    "destination"
                };
                ...
                // Rule A: byte-preserving RX — decode succeeds and the
                // original packet comes back verbatim.
                let decoded_len = lichen_schc::decode_rule255(
                    &rx_input,
                    &mut decoded,
                    usize::MAX,
                )
                .expect("byte-preserving decode must accept structural-rule255 rows");
                assert_eq!(&decoded[..decoded_len], &wire[..], "{}: decoded != wire", name);
            }
    ...
    if rule255_rx_byte_preserving_address_count != expected.rule255_rx_byte_preserving_address {
        failures.push(format!(
            "Expected {} Rule 255 RX structural-reject vectors (per metadata), found {}",
            expected.rule255_rx_byte_preserving_address, rule255_rx_byte_preserving_address_count
        ));
    }
