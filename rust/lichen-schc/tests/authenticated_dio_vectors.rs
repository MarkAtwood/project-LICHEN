...
fn authenticated_dio_vectors_drive_production_admission() {
    // The canonical test/vectors/authenticated_schc_dio.json corpus was
    // regenerated by the canonical generator after the upstream AddrForKey
    // migration (i72x.6): root/victim DODAGIDs carry upstream addresses
    // (validated against live yggdrasil-go); the 9 fixed-peer cases are
    // byte-identical synthetic-DODAGID cases.
