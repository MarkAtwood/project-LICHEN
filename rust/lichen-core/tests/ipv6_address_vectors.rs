...
fn key_derived_identity_binds_link_local_and_upstream_native() {
    ...
    // primary/native 0200::/8 fields holding UPSTREAM Yggdrasil AddrForKey
    // values — the settled profile (spec/decisions.jsonl
    // upstream-yggdrasil-addressing). The REJECTED SHA-512 native profile
    // lives only in test/vectors/legacy/ipv6_addresses_native_sha512.json
    // (QUARANTINED — quarantine-integrity pin only, never a conformance
    // oracle). Post-migration the routable address does NOT embed the IID, so
    ...
    for vector in document["vectors"].as_array().expect("vectors array") {
        ...
        // external oracle (yggdrasil-go@422836ee). The live corpus fields
        // must carry these same upstream values (conformance), and must NOT
