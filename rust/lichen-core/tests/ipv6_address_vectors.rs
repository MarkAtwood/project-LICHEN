...
fn key_derived_identity_binds_link_local_and_upstream_native() {
    // Merge reconciliation: post-migration name from beads-worker-8 (the live
    // corpus now carries the upstream profile, so "quarantined_native" no
    // longer describes what this test binds). Comment merges both sides:
    // HEAD's field enumeration + worker-8's commit/decision references and
    // the iid_in_native semantics.
    ...
    for vector in document["vectors"].as_array().expect("vectors array") {
        ...
        // Merge reconciliation: HEAD's byte-form corpus checks kept here at
        // the top of the loop; worker-8's equivalent string-form check near
        // the `native` assert below is subsumed by this one (byte equality
        // against the same pinned oracle). Worker-8's corpus-flag assertion
        // is kept alongside: the byte-level assert_ne pins substantive
        // non-embedding, the flag assert pins the corpus's own bookkeeping.
        // The legacy-corpus fetch stays at the quarantine section below
        // (worker-8's relocation) — it is only used there.
...
fn yggdrasil_derivation_corpus_matches_upstream_addr_for_key() {
    ...
    for entry in entries {
        ...
        // Merge reconciliation: worker-8's strict match is correct — the
        // corpus retired the plain "binding_invariant" spelling before the
        // merge base, so HEAD's extra disjunct could never match.
