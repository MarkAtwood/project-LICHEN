...
//
// The receiver joins the relay-rooted DODAG first (bare-DIO join, the
// join_leaf shape), then receives a carrier DIO carrying the signed 0x17
// option under two clocks: past expiry and the clockless default — both
// must be admitted as Baseline (treated as unsigned per spec L679),
// never rejected.
//
...
// is structurally impossible with the shared vector (no private key);
// the cross_check itself is covered by root_sig.rs unit tests.
