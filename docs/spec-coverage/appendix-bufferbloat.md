...
bufferbloat_congestion.json, consumed by the Rust and Python test suites;
the C forwarding suite has equivalent focused assertions. Gap beads filed
...
| R-ABF-011 | NACK to mesh source for mesh-forwarded packets when full (historical Design Principles 4+5 wording) | superseded | Settled project decision requires local backpressure recording when no eligible protocol response exists; no unsolicited mesh NACK is emitted. C/Python/Rust record queue-full/backpressure locally and return the local error. The standalone ICMPv6 helper vector remains a component test, not a forwarding side effect. |
