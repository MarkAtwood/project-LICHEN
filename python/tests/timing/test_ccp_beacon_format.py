...
def test_verify_gate_accepts_valid_signature() -> None:
    ...
    signed = header + b"\x01\x02\x03"
    sig = schnorr48.sign(identity.privkey, identity.pubkey, signed)
    beacon = signed + sig
    assert len(beacon) >= MIN_BEACON_SIZE
    assert verify_gate(beacon, lambda s, sig: schnorr48.verify(
        identity.pubkey, s, sig)) is True
