...
def simulate_traffic(
    ...
    # min_tx_interval unused (kept for parity with mac_study)
    _ = AIRTIME_S / 0.01  # 1% duty cycle
    ...
    while time_s < duration_s or pending_transmissions:
        ...
        if time_s < duration_s:
            for node in nodes:
                if rng.random() < (
                    slot_s / 60
                ) * msg_rate_per_node_per_min and node.routing_table:
...
def run_sf_study():
    ...
    print("=" * 70)
    print(
        f"{'Nodes':>6} {'MsgRate':>8} {'Delivery':>10} "
