...
class SosResource(resource.ObservableResource):
    ...
    **POST** activates with ``{"type":"sos", "node":..., "ts":...}`` plus the
    ...
    {"from","t"} core fields).  Per spec 18.4.1 the origin signature is
        ...
        and the origin sequence must strictly advance. A missing, malformed or
        invalid origin signature is SILENTLY DROPPED (spec 18.4.1: no error
        response, preventing attacker enumeration).
    **DELETE** cancels.  **GET** and **Observe** expose the current state to all
    ...
    async def render_post(self, request: Message) -> Message:
        ...
        if (
            ...
            return Message(code=aiocoap.BAD_REQUEST)
        # Spec 18.4.1: SOS MUST carry a valid origin signature; a missing,
        # malformed or invalid signature is SILENTLY DROPPED (no error
        # response). The envelope carries the signer's pubkey (32 B) and the
        # wire origin signature (8 B seq + 48 B sig).
