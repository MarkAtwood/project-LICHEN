...
static void test_verify_gate(void)
{
	...
	/* Merge fix (main + beads-worker-4): CHECK -> assert, matching this
	 * file's idiom (worker-4's branch never compiled this function:
	 * CHECK is only defined in the sibling top-level main.c). The four
	 * gate conditions are unchanged. */
	assert(lichen_beacon_verify_gate(beacon, sizeof(beacon), stub_verify,
					 (void *)1));
	assert(!lichen_beacon_verify_gate(beacon, LICHEN_BEACON_MIN_SIZE - 1U,
					  stub_verify, (void *)1));
