...
/* IID literals from test/vectors/yggdrasil-derivation.json (SHA-512 IID,
 * link-local only, unchanged). Native-address literals are upstream
...
static const struct iid_vector vectors[] = {
	...
	{
		/* Upstream address_test.go TestAddress_AddrForKey anchor. */
		.pubkey = {
		...
		.iid = {0xcd, 0x97, 0x71, 0xd9, 0xdd, 0x74, 0x59, 0x46},
		.native = {
...
int main(void)
{
	...
	for (size_t i = 0; i < sizeof(vectors) / sizeof(vectors[0]); i++) {
		/* The routable address does NOT embed the SHA-512 IID: the
		 * settled upstream-yggdrasil-addressing decision keeps the
		 * IID link-local-only, so no addr[8:16]==iid check here. */
