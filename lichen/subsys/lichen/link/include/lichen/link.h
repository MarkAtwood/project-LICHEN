...
/* Merge resolution: beads-worker-1 added lichen_slot_map_tx_allowed and
 * closed the CONFIG_LICHEN_TDMA block here; HEAD keeps the block open so
 * lichen_tdma_guard_budget_sufficient stays inside the gate (its tdma.c
 * definition is gated the same way). Intents are compatible: keep the new
 * declaration, drop worker-1's early #endif — the block closes after
 * lichen_tdma_guard_budget_sufficient below. */
