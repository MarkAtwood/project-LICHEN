...
static uint32_t s_last_post_ms;
...
static int deaddrop_post(struct coap_resource *resource, struct coap_packet *request, struct sockaddr *addr, socklen_t addr_len)
{
	...
	if (now < s_last_post_ms || (now - s_last_post_ms) < 60000U) {  /* wrap-safe */
		return COAP_RESPONSE_CODE_SERVICE_UNAVAILABLE; /* 1 msg/60s rate limit */
	}
	s_last_post_ms = now;
	k_mutex_lock(&s_dtn_buf_mutex, K_FOREVER);
	k_mutex_lock(&s_senml_pack_mutex, K_FOREVER);
...
static int deaddrop_get(struct coap_resource *resource, struct coap_packet *request, struct sockaddr *addr, socklen_t addr_len)
{
	...
	return coap_respond(resource, request, addr, addr_len, COAP_RESPONSE_CODE_CONTENT, buf, len);
}
