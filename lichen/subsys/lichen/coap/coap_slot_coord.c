		      ...
		      struct sockaddr *addr, socklen_t addr_len)
{
	...
	if (result != LICHEN_CLAIM_ACCEPTED) {
		...
		if (result == LICHEN_CLAIM_REJECT_CONFLICT &&
			 ...
			 * conflict_cose_len is a plain size_t (in-out value
			 * parameter), so the length checks test it directly
			 * (bead 3yhv: the old *conflict_cose_len forms were
			 * invalid unary-* on a size_t). */
